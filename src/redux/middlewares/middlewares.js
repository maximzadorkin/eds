import { ApiSpecs, ApiSpecsLabels, ApiStatistic, ApiTypes } from '../../Api.js'
    ...
    if (action.type === NEXT_LAYER && layer === DATE_RANGES) {
        ...
        const request = {
            ...
            Periods: state.dateRanges.map(dr => ({
                From: `${dr.startDate.getFullYear()}-${dr.startDate.getMonth() + 1}-${dr.startDate.getDate()}`,
                To: `${dr.endDate.getFullYear()}-${dr.endDate.getMonth() + 1}-${dr.endDate.getDate()}`,
            }))
        }
        console.log(JSON.stringify(request, null, ' '))
        const response = await fetch(ApiStatistic, {method: 'POST', body: JSON.stringify(request)})
        let data = []
